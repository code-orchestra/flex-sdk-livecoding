...
public final class NodeFactory implements ErrorConstants
{
	...
	public Node assignmentExpression(Node lhs, int op, Node rhs, int pos)
	{
        ...
        if (op != ASSIGN_TOKEN)
		{
			...
			op = op == MULTASSIGN_TOKEN ? MULT_TOKEN :
				op == DIVASSIGN_TOKEN ? DIV_TOKEN :
				op == MODULUSASSIGN_TOKEN ? MODULUS_TOKEN :
				op == PLUSASSIGN_TOKEN ? PLUS_TOKEN :
				op == MINUSASSIGN_TOKEN ? MINUS_TOKEN :
				op == LEFTSHIFTASSIGN_TOKEN ? LEFTSHIFT_TOKEN :
				op == RIGHTSHIFTASSIGN_TOKEN ? RIGHTSHIFT_TOKEN :
				op == UNSIGNEDRIGHTSHIFTASSIGN_TOKEN ? UNSIGNEDRIGHTSHIFT_TOKEN :
				op == BITWISEANDASSIGN_TOKEN ? BITWISEAND_TOKEN :
				op == BITWISEXORASSIGN_TOKEN ? BITWISEXOR_TOKEN :
				op == BITWISEORASSIGN_TOKEN ? BITWISEOR_TOKEN :
				op == LOGICALANDASSIGN_TOKEN ? LOGICALAND_TOKEN :
				op == LOGICALXORASSIGN_TOKEN ? LOGICALXOR_TOKEN :
				op == LOGICALORASSIGN_TOKEN ? LOGICALOR_TOKEN : ERROR_TOKEN;
