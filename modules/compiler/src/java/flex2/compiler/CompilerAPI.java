...
public final class CompilerAPI {
    ...
    private static ExecutorService threadPull;
    private static final Object threadPullMonitor = new Object();

    private static synchronized void prepareThreadPull() {
        synchronized (threadPullMonitor) {
            if (threadPull == null || threadPull.isShutdown()) {
                threadPull = Executors.newFixedThreadPool(100);
            }
        }
    }
            ...
            commands.add(new Callable() {
                ...
                public Object call() throws Exception {
                    if (!threadPull.isShutdown()) {
                        ...
                        for (int j = units.size(); j < finalI + 1; j++) {
                            units.add(null);
                        }
    ...
    private static boolean analyze(final List<Source> sources, final flex2.compiler.SubCompiler[] compilers,
        ...
        for (int i = start; i < end; i++) {
            ...
            commands.add(new Callable() {
                ...
                public Object call() throws Exception {
                    if (!threadPull.isShutdown()) {
                        Source source = sources.get(finalI);
                        CompilationUnit u = source.getCompilationUnit();
