...
public class SwcGroup
{
	...
	{
		return swcs.values().stream().map(Swc::getLastModified).collect(Collectors.toList());
	}
	...
	private void updateNameMappings()
	{
		for (Swc swc : swcs.values()) {
			...
			while (iter2.hasNext()) {
				...
				boolean success = nameMappings.addClass(namespaceURI, name, className);
				if (!success) {
	...
	private void updateMaps()
	{
		...
		Arrays.sort(scriptArray, (swcScript1, swcScript2) -> {
				...
				if (newerSwcScript != null) {
					...
					if (compilationUnit != null) {
						...
						if (newerCompilationUnit != null) {
							if ((compilationUnit.typeInfo != newerCompilationUnit.typeInfo)) {
